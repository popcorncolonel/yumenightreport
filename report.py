...
class Report(ndb.Model):
    ...
    * variable name and key name need to be the same
    ...
    def get_dreamers_this_year(self):
        if self.is_past_2017():
            return sum(report.get_dreamers() for report in self.get_previous_reporters())
        else:
            return 0 # didn't support this pre-2018

    def get_customers_this_year(self):
        if self.is_past_2017():
            return sum(report.get_customers_today() for report in self.get_previous_customers())
        else:
            return self.customers_this_year

    def get_achievement_rate(self):
        if self.is_past_2017():
            if self.dreams is None or self.daily_dream_goal:
                return None
            else:
                if self.daily_dream_goal == 0:
                    return 100.
                achievement_rate = (dreams / float(self.daily_dream_goal)) * 100
    ...
    def get_daily_dream_goal(self):
        ...
        working_days_left_in_year = get_working_days_left_in_year(self.date.date())
        if working_days_left_in_year == 0:
            return 0.
        if self.is_past_2017():
            total_dreams_for_year = Report.get_dreams_for_year(self.date)
        else:
            total_dreams_for_year = self.dreams_this_year
        #dreams_remaining = max(0, self.yearly_dream_goal - total_dreams_for_year)
        dreams_remaining = 0  # TODO: temporarily broken
